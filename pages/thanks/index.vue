<template>
  <div class="container">
    <div class="flex flex-col items-center gap-5 py-24 text-center md:gap-6 lg:gap-7">
        <!-- title -->
        <h1 class="text-4xl font-bold sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl text-primary" v-if="thanks.title.length > 0">
        ...
        <!-- orderCode -->
        <!-- text thank you -->
        <p class="text-2xl font-semibold text-gray-700 md:text-3xl lg:text-4xl" v-if="thanks.description.length > 0">{{ thanks.description }}</p>
        <!-- text thank you -->
        <!-- button -->
        <nuxt-link to="/shop" class="inline-flex px-4 py-3 text-sm font-medium tracking-wider rounded-full shadow-sm md:px-6 md:py-3 bg-primary text-gray-50 hover:shadow-lg" v-if="thanks.buttons.back_to_shop_text.length > 0"> 
