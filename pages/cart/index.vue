    ...
    <div class="container my-6">
            ...
            <div v-if="items.length > 0">
                ...
                <div class="flex flex-wrap justify-between lg:flex-nowrap">
                    ...
                    <div class="w-full lg:w-4/6 ">
                        <!-- Cart title -->
                        <h2 class="px-4 mb-6 text-3xl font-light md:text-4xl guard-cairo-font">{{$settings.sections.cart.title}}</h2>
                        <!-- Cart title -->
                        <!-- cart -->
                    ...
                    <!-- Products -->
                    <!-- Price -->
                    <div class="w-full mx-4 mb-8 lg:w-2/6 lg:mt-14 md:mb-6">
                        <div v-if="!loading.cart && items.length > 0" class="flex flex-col p-2 box-shadow-x rounded-2xl">
                            <!--  -->
                            ...
                            <div class="flex flex-col p-2">
                                <a href="/checkout2" class="flex items-center justify-center px-4 py-2 mb-4 text-xl text-white border-2 rounded-full bg-primary bg-primary-border scale hover:opacity-80">
                                    <span class="overflow-hidden text-base font-bold whitespace-nowrap overflow-ellipsis">{{ $settings.sections.cart.buttons.checkout_text }}</span>
                                </a>
                                <nuxt-link to="/shop" class="flex items-center justify-center px-4 py-2 text-xl border-2 rounded-full text-primary bg-primary-border scale hover:opacity-80">
                                    <span class="overflow-hidden text-base font-bold whitespace-nowrap overflow-ellipsis">{{ $settings.sections.cart.buttons.back_to_shop_text }}</span>
                                </nuxt-link>
                            </div>
                            <!--  -->
                        </div>
                ...
                <div class="flex flex-col mt-6 bg-white" v-if="!loading.cart && items.length > 0">
                    ...
                    <div class="flex flex-wrap" v-if="upsells.length > 0">
                        <template v-for="(upsell, i) in upsells">
                            <div v-for="(product, ii) in upsell.products" :key="ii+'_'+i" class="w-1/2 p-2 md:w-1/3 lg:w-1/5">
        ...
        <div class="flex justify-center p-2">
            <div v-if="!loading.cart && items.length == 0" class="flex flex-col items-center">
                <h2 class="w-full mb-3 text-center">{{ $settings.sections.cart.empty_text }}</h2>
