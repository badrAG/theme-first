<template>
    <div class="container my-6">
        <div v-if="item" class="flex flex-wrap items-center bg-white border border-gray-200 rounded-xl transition-all ease-linear box-shadow-xs-hover mx-4">
            
            <div class="w-full md:w-1/3">
                <div class="pb-4/5 relative">
                    <nuxt-link :to="`/shop/${item.slug}`" :title="item.title" :aria-label="item.title">
                        <si-image width="400" height="400" class="h-full w-full rounded-t-xl md:rounded-t-none md:rounded-l-xl absolute inset-0 object-cover" :src="item.image ? item.image.src : null" :alt="item.name" srcset=""/>
                    </nuxt-link>
                </div>
            </div>
            ...
            <div class="w-full md:w-2/3 p-4">
                <div class="flex flex-col justify-start items-start">
                    <nuxt-link :to="`/shop/${item.slug}`" :title="item.title" :aria-label="item.title">
                        <h1 class="guardian-font text-2xl md:text-3xl font-bold mb-4 ">{{ item.name }}</h1>
                    </nuxt-link>
