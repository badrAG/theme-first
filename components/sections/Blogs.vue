<template>
  <div class="container py-4">
    <!-- blogs title -->
    <div class="flex flex-col items-center p-2 px-4 mb-4 md:justify-between md:flex-row" v-show="( blogs.title.length > 0 && blogs.show_more_text.length > 0 ) || ( blogs.title.length > 0 || blogs.show_more_text.length > 0 )">
      <div v-show="blogs.title.length > 0">
        <h2 class="text-xl font-bold align-center md:text-2xl">{{ blogs.title }}</h2>
      </div>
      <div v-show="blogs.show_more_text.length > 0">
        <nuxt-link to="blog">
          <span class="text-sm font-normal underline text-gr hover-text-bg">{{ blogs.show_more_text }}</span>
        </nuxt-link>
      </div>
    </div>
    <!-- blogs title -->
...
export default {
    data() {
        return {
          loading: true,
          blogs: this.$settings.sections.blogs,
          items: []
        }
    },
    async fetch() {
        try{
          this.loading = true;
          if(this.$settings.sections.blogs.items.length > 0) {
            this.items = this.$settings.sections.blogs.items;
          } else {
