<template>
  <div class="container py-6">
    <div class="p-2 px-4 mb-6" v-show="$settings.sections.collections_products.title.length > 0">
        <h3 class="align-center font-bold text-xl md:text-2xl">{{ $settings.sections.collections_products.title }}</h3>
    </div>
          ...
          <nuxt-link :to="`/shop/${collection.slug}`" :title="collection.name" :aria-label="collection.name">
            <div class="icon-box flex items-center justify-between px-4 py-3">
              <div class="flex items-center">
                <h2 class="products-collections-name text-sm lg:text-base font-bold" :title="getFormattedCollectionName(collection.name)">{{ getFormattedCollectionName(collection.name) }}</h2>
                <span class="w-2"></span>
                <fa class="icon text-sm lg:text-base text-primary translate" :icon="['fa', 'arrow-right']"></fa>
              </div>
              <span class="text-md font-medium text-gr">{{ getProductsCount(index) }} {{ $settings.sections.collections_products.items_title }}</span>
            </div>
          </nuxt-link>
    ...
    <div v-if="!loading.collections && collections.length==0" class="flex flex-wrap items-center  bg-white p-2 mx-4 border rounded-xl box-shadow">
