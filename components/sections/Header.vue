        ...
        <header class="container flex items-center justify-between w-full ">
            ...
            <div class="icons flex-shrink-0 mx-1">
                ...
                <div class="lang flex items-center justify-between">
                                    ...
                                    <div class="py-1" v-for="(item,i) in item.childrens" :key="i" >
                                        ...
                                        <ul v-if="item.childrens && item.childrens.length > 0">
                                            <li class="px-2 pt-2" v-for="(child,ii) in item.childrens" :key="ii">
                                                <a class="flex text-sm" :href="child.url">
                                                    {{ child.text }}
                                                </a>
                                            </li>
                                        </ul>
                    ...
                    <div class="user p-3 rounded-full transition-all ease-linear delay-200 hover:bg-gray-200">
                        <router-link v-if="$settings.sections.header.icons.account" to="/account/orders" title="Account" class="flex flex-col justify-center">
                    ...
                    <div class="heart p-3 rounded-full transition-all ease-linear delay-200 hover:bg-gray-200">
                        <router-link v-if="$settings.sections.header.icons.wishlist" to="/wishlist" title="Wishlist" class="flex flex-col justify-center relative">
                            ...
                            <small v-if="$store.state.wishlist.length>0"  class="-top-3 -right-2 rounded-full absolute w-5 h-5 p-1 bg-red-400 text-white flex justify-center items-center text-xs">{{ $store.state.wishlist.length }}</small>
                        </router-link>
                    </div>
                    <div class="cart p-3 rounded-full transition-all ease-linear delay-200 hover:bg-gray-200">
                        <router-link v-if="$settings.sections.header.icons.cart" to="/cart" title="Cart" id="cart-icon" class="flex flex-col justify-center relative">
                            ...
                            <small v-if="$store.state.cart.length>0" class="-top-3 -right-2 rounded-full absolute w-5 h-5 bg-red-400 text-white flex justify-center items-center text-xs">{{ $store.state.cart.length }}</small>
                        </router-link>
                    </div>
            ...
            <ul v-for="(item, i) in menu.items" :key="i" class="flex flex-col">
                <li class="flex items-center justify-between pt-2 px-3">
                    <router-link class="flex" :to="item.url">
                        <div class="hover:underline px-1">{{ item.text }}</div>
                    </router-link>
                    <button  @click="activeId = activeId != item._id ? item._id : null">
                ...
                <transition name="slide">
                    <div class="relative">
                        <div v-if="item._id == activeId" class="top-2 absolute shadow-lg z-20 border header-bg p-4">
                            <div class="py-1" v-for="(item,i) in item.childrens" :key="i">
                                <router-link class="flex hover:underline text-sm" :to="item.url">{{item.text}}</router-link>
                                <ul v-if="item.childrens && item.childrens.length > 0">
                                    <li class="px-2 pt-2" v-for="(child,ii) in item.childrens" :key="ii">
                                        <nuxt-link  class="flex hover:underline text-sm" :to="child.url">
                                        {{ child.text }}
                                        </nuxt-link>
                                    </li>
                                </ul>
