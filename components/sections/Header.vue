    ...
    <div class="header-bg header-text-color  border-b-2" :class="menu? 'pt-2' : 'py-2'">
        <header class="container flex items-center justify-between w-full ">
            ...
            <div class="mx-4">
                ...
                <div class="logo">
                    <router-link to="/">
                        <si-image width="70" height="50" property="height" class="h-8 md:h-11 w-full object-contain" :src="section.logo ? section.logo.src : $store.state.defaults.logo" alt="Store logo"/>
                    </router-link>
                </div>
                <!-- logo -->
            </div>
            ...
            <div class="flex items-center " :class="$settings.sections.header.icons.search? 'w-full active-search':'unactive-search'">
                <!-- show menu botton  -->
                <div class="headermenu-button mx-1 p-3 rounded-full hidden transition-all ease-in-out delay-150 hover-bg">
            ...
            <div class="icons flex-shrink-0 mx-1">
                ...
                <div class="lang flex items-center justify-between">
                    ...
                    <!-- courency and language dropdown  -->
                    <div v-if="$settings.sections.header.icons.account" class="user p-3 rounded-full transition-all ease-in-out delay-150 hover-bg">
                        <router-link  to="/account/orders" title="Account" class="flex flex-col justify-center">
                            <svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="user" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" style="height: 18px; width: 18px;" class="translate svg-color"><path  d="M313.6 304c-28.7 0-42.5 16-89.6 16-47.1 0-60.8-16-89.6-16C60.2 304 0 364.2 0 438.4V464c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48v-25.6c0-74.2-60.2-134.4-134.4-134.4zM400 464H48v-25.6c0-47.6 38.8-86.4 86.4-86.4 14.6 0 38.3 16 89.6 16 51.7 0 74.9-16 89.6-16 47.6 0 86.4 38.8 86.4 86.4V464zM224 288c79.5 0 144-64.5 144-144S303.5 0 224 0 80 64.5 80 144s64.5 144 144 144zm0-240c52.9 0 96 43.1 96 96s-43.1 96-96 96-96-43.1-96-96 43.1-96 96-96z" class=""></path></svg>
                        </router-link>
                    </div>
                    <div v-if="$settings.sections.header.icons.wishlist" class="heart p-3 rounded-full transition-all ease-in-out delay-150 hover-bg">
                        <router-link  to="/wishlist" title="Wishlist" class="flex flex-col justify-center relative">
                            <svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="heart" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5 translate svg-color"><path d="M12,21C10.349,21,2,14.688,2,9,2,5.579,4.364,3,7.5,3A6.912,6.912,0,0,1,12,5.051,6.953,6.953,0,0,1,16.5,3C19.636,3,22,5.579,22,9,22,14.688,13.651,21,12,21ZM7.5,5C5.472,5,4,6.683,4,9c0,4.108,6.432,9.325,8,10,1.564-.657,8-5.832,8-10,0-2.317-1.472-4-3.5-4-1.979,0-3.7,2.105-3.721,2.127L11.991,8.1,11.216,7.12C11.186,7.083,9.5,5,7.5,5Z"></path></svg>
        ...
        <nav v-if="menu" class="flex items-center justify-center font-normal text-sm ">
            <ul v-for="(item, i) in menu.items" :key="i" class="flex flex-col" @mouseover="activeId = activeId = item._id" @mouseleave="activeId = activeId = null">
                <li class="flex items-center p-2 mx-2 border-b-2 border-transparent hover:border-black transition-all delay-150 ease-linear">
                    <router-link :to="item.url">
                        {{ item.text }}
                    </router-link>
                    <button v-if="item.childrens.length > 0" class="chivron ml-1"  @click="activeId = activeId != item._id ? item._id : null">
                        <fa class="text-xs transform" v-if="item.childrens && item.childrens.length > 0"  :icon="['fa', 'angle-down']" :class="[activeId==item._id ? 'rotate-180 transition-all delay-150 ease-linear' : ''] "></fa>
                    </button>
                </li>
                <transition name="slide">
