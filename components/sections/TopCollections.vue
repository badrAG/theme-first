<template>
    <div class="">
        <!-- title -->
        <div class="collection-bg" v-show="$settings.sections.top_collections.title.length > 0">
            <div class="container mx-auto">
                <h1 class="px-4 pt-4 pb-2 text-2xl font-light text-center md:text-3xl guard-cairo-font collection-text-bg">{{ $settings.sections.top_collections.title }}</h1>
            </div>
        </div>
        <!-- title -->
        <!-- collections -->
        <div class="relative">
            <div class="absolute w-full md:h-2/5 collection-bg md:" :class="items.length > 3? 'h-2/3' : 'h-2/5'"></div>
            <div class="mx-auto small-container">
                <ul class="flex flex-wrap justify-center p-4">
                    <li class="flex flex-col items-center w-1/3 mb-5 collection-box md:mb-6 md:w-1/6" v-for="(item,i) in items" :key="i" >
                        <nuxt-link class="mb-3" :to="item.childrens.length > 0 ? `/collections/${item.slug}` : `/shop/${item.slug}`">
                            <si-image class="relative object-cover w-24 h-24 overflow-hidden rounded-full img-zom md:h-28 md:w-28" :src="item.image ? item.image.src : null" :alt="item.name" srcset=""/>
                        </nuxt-link>
                        <nuxt-link class="top-collections-name line w-28" :to="item.childrens.length > 0 ? `/collections/${item.slug}` : `/shop/${item.slug}`">
                            <div class="relative text-sm font-bold text-center collection-text md:text-base">{{ item.name }}</div>
                        </nuxt-link>
                    </li>
                </ul>
            </div>
        ...
        <div class="container">
            <div v-if="!loading && items.length==0" class="flex flex-wrap items-center p-2 mx-4 bg-white border rounded-xl box-shadow">
